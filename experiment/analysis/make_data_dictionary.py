"""Generate processed/data_dictionary.md from the schema and the data itself.

Fails if runs.csv gains a column that nobody has documented.
"""
...
HEADER = """# Data dictionary - `processed/runs.csv`

One row per executed run. This campaign: **{n} confirmatory runs**, 24 cells
(4 scenarios x 6 arms) x {per} repetitions.
...
def main() -> None:
    ...
    undocumented = [c for c in df.columns if c not in META]
    if undocumented:
        raise SystemExit(f"undocumented columns in runs.csv: {undocumented}")
    rows = "".join(f"| `{c}` | {META[c][0]} | {META[c][1]} | {META[c][2]} | `{META[c][3]}` |\n"
                   for c in df.columns)
    tail = f"""
## Censoring

`t_service_restore` is right-censored in **{int(df.censored_restore.sum())} of {len(df)}** runs
...
`exclusion_flag = 1` in **{int((df.exclusion_flag == 1).sum())}** runs. No run was excluded.
...
"""
    out = ROOT / "processed" / "data_dictionary.md"
    out.write_text(HEADER.format(n=len(df), per=len(df) // 24) + rows + tail)
    print(f"  documented {len(df.columns)} columns -> {out.relative_to(ROOT)}")
