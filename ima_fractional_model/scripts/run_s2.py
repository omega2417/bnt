#!/usr/bin/env python3
"""Run scenario S2 and save its outputs (CSV/NPZ/JSON)."""
...
def main():
    print_header("Scenario S2")
    graph = load_graph(graph_config())
    cfg = load_scenario_config(CONFIG_DIR / "scenario_S2.yaml")
    result = scenario_from_config(graph, cfg)
    meta = save_scenario_outputs(result, RESULTS_DIR / "scenarios", "S2")
    ...
    print(f"  outputs -> results/scenarios/S2_*.{{csv,npz,json}}")
    return result
