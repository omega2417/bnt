...
namespace ustfuse {
static double g_t_encode=0, g_t_ensemble=0, g_t_assoc=0;
using Clock=std::chrono::high_resolution_clock;
static double us_since(std::chrono::time_point<Clock> a){return std::chrono::duration<double,std::micro>(Clock::now()-a).count();}
...
ClassificationResult classify(const Classifier& clf,
    ...
    for (int e = 0; e < kEnsemble; ++e) {
        preds[e] = clf.memberSoftmax(clf.members[e], x, clf.temperature);
        for (int c = 0; c < kNumClasses; ++c) mean[c] += preds[e][c] / kEnsemble;
    }
    ...
    for (int e = 0; e < kEnsemble; ++e) {
        ...
        expectedMemberEntropy += h / kEnsemble;
    }
...
struct Engine {
    ...
    static double inflation(double quality) {
        double base = 9.0;                  // baseline positional variance (m^2)
    ...
    void step(int scenarioId, int tick, std::vector<Measurement>& tickMeas,
        ...
        for (size_t i = 0; i < tickMeas.size(); ++i) {
            ...
            g_t_encode += us_since(_te);
            auto _ts = Clock::now();
            cls[i] = classify(clf, xd, tickMeas[i].quality);
            g_t_ensemble += us_since(_ts);
        ...
        for (auto& t : tracks) {
            ...
            for (size_t i = 0; i < tickMeas.size(); ++i) {
                ...
                double lambda = 1.0 / (1.0 + 3.0 * cls[i].epistemic);
                double w = kin * ((1.0 - lambda) + lambda * (0.25 + 0.75 * sem));
...
int main(int argc, char** argv) {
    ...
    std::ofstream tf("ustfuse_timing.csv");
    tf << "scenario_id,tick,n_meas,n_tracks,encode_us,ensemble_us,assoc_us,total_us\n";
    long globalTick=0;
    size_t i = 0;
    long numScenarios = 0;
    while (i < meas.size()) {
        ...
        while (i < meas.size() && meas[i].scenarioId == scenarioId) {
            ...
            int ntr=(int)engine.tracks.size();
            g_t_encode=g_t_ensemble=g_t_assoc=0;
            auto _t0=Clock::now();
            engine.step(scenarioId, tick, tickMeas, trackOut, clsOut);
            double tot=us_since(_t0);
            if (globalTick>=100)
                tf<<scenarioId<<','<<tick<<','<<tickMeas.size()<<','<<ntr<<','
                  <<g_t_encode<<','<<g_t_ensemble<<','<<g_t_assoc<<','<<tot<<'\n';
            ++globalTick;
        }
    }
    tf.close();
